...
class QuizBio extends StatefulWidget{
  const QuizBio({Key? key}) :super(key: key);
  @override
  _QuizBioState createState() => _QuizBioState();
}

class _QuizBioState extends State<QuizBio> with TickerProviderStateMixin{
  var db = DBconnect2();
      ...
      builder: (ctx, snapshot){
        if(snapshot.connectionState== ConnectionState.done){
          if(snapshot.hasError){
            ...
            return Scaffold(
              appBar: AppBar(
                title: const Text('Biologi',style: TextStyle(color: Colors.black),),
                centerTitle: true,
